...
func main() {
	if os.Getenv("GITEA_SERVER__PROTOCOL") == "https" {
		createCerts()
	}
...
func createCerts() {
	...
	if len(os.Getenv("GITEA_SERVER__ROOT_URL")) > 0 {
		certHosts = append(certHosts, os.Getenv("GITEA_SERVER__ROOT_URL"))
	}
...
func writeConfigFromEnvVars() {
	...
	validConfigRE := regexp.MustCompile(`^GITEA_(?:([^=]+)__)?([^=]+)=(.+)$`)
